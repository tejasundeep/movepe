const { storage, analyticsStorage } = require('../storage');

class AnalyticsService {
  constructor() {
    // No need to initialize events in memory as we'll use Prisma storage
  }
  ...
  async trackEvent(eventType, eventData, userId = null) {
    try {
      ...
      // Store the event using Prisma
      const createdEvent = await analyticsStorage.create(event);
   ...
   * @returns {Promise<Array>} All analytics events
   */
  async getAllEvents() {
   ...
   * @returns {Promise<Array>} Filtered analytics events
   */
  async getEventsByType(eventType) {
    return await analyticsStorage.getByEventType(eventType);
  }
  ...
  async getEventsByUser(userId) {
    return await analyticsStorage.getByUserId(userId);
  }
}

...
class VendorService {
   ...
   * @param {string} email - The vendor's email
   * @param {string} status - The new availability status ('available', 'busy', 'vacation')
  ...
  async updateAvailability(email, status) {
    ...
    await analyticsService.trackEvent('vendor_availability_changed', {
      vendorId: vendors[vendorIndex].vendorId,
      availability: status
    });
  ...
  async getAffiliateStats(email) {
    ...
    return {
      isAffiliate: affiliate.isAffiliate || false,
      referralCode: affiliate.referralCode || '',
      referredVendors: affiliate.referredVendors || [],
      activeReferrals,
      discountedCommissionsRemaining: affiliate.discountedCommissionsRemaining || 0,
  ...
  async createCrossLead(leadData, vendorEmail) {
    ...
    // Update vendor's cross-lead count and earnings if applicable
    await storage.updateData('vendors.json', (vendors) => {
      const vendorIndex = vendors.findIndex(v => v.email === vendorEmail);
      if (vendorIndex !== -1) {
        // Initialize cross-leads property if it doesn't exist
        if (!vendors[vendorIndex].crossLeads) {
          vendors[vendorIndex].crossLeads = {
            count: 0,
            earnings: 0,
            history: []
          };
        ...
        // Increment count
        vendors[vendorIndex].crossLeads.count += 1;
        
        // Add to history
        vendors[vendorIndex].crossLeads.history.push({
          orderId,
          customerEmail,
          createdAt: now,
          status: 'Created'

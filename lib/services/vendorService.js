import { vendorStorage, orderStorage, userStorage } from '../storage';
...
class VendorService {
  ...
  async createVendor(vendorData) {
    ...
    const newVendor = {
      id: vendorId,
      userId: vendorId, // In our schema, userId is the same as vendorId
      businessName: name,
    ...
    const createdVendor = await vendorStorage.create(newVendor);
    ...
    await analyticsService.trackEvent('vendor_created', {
      ...
      name: createdVendor.businessName,
      services: [],
      serviceAreas: createdVendor.serviceAreas
    });
    ...
    return {
      ...
      email,
      serviceAreas: createdVendor.serviceAreas,
      basePrice: createdVendor.basePrice,
      description: createdVendor.description,
  ...
  async getVendorByEmail(email) {
    ...
    // Then get the vendor by userId
    return await vendorStorage.getById(user.id);
  }
  ...
  async getVendorById(vendorId) {
    return await vendorStorage.getById(vendorId);
  }
  ...
  async updateAvailability(email, status) {
    ...
    const updatedVendor = await vendorStorage.update(vendor.id, {
      // We don't have an availability field in the Prisma schema,
      // so we'll store it in the specialties field for now
      specialties: JSON.stringify([status]),
      updatedAt: new Date()
    });
  ...
  async getVendorRequests(vendorId) {
    ...
    const lostOpportunitiesWithQuote = normalizedOrders.filter(order => 
      ...
      const selectedVendor = vendors.find(v => v && v.id === order.selectedVendorId);
      ...
      return {
        ...
        didQuote: true,
        selectedVendorName: selectedVendor?.businessName || 'Another vendor',
  ...
  async getAffiliateStats(email) {
    ...
    const crossLeadsSubmitted = orders.filter(order => 
      order.crossLead && order.crossLeadVendorId === vendor.id
    ).length;
        ...
        if (hasCompletedOrders) {
          ...
          if (referredVendor) {
            activeReferrals.push({
              ...
              name: referredVendor.businessName,
    ...
    return {
      ...
      referralCode: affiliate.referralCode || vendor.id.substring(0, 8),
  ...
  async createCrossLead(leadData, vendorEmail) {
    ...
    const newOrder = {
      ...
      vendorRequests: vendorManaged ? [vendor.id] : [],
      quotes: [],
      crossLead: true,
      crossLeadVendorId: vendor.id,
      vendorManaged: !!vendorManaged,
      isCrossLead: true,
      referringVendorId: vendor.id,
      commissionRate: 5 // 5% commission discount
    };

    // Add the order to orders.json
    const updated = await orderStorage.create(newOrder);
    ...
    await this.trackCrossLeadSubmission(vendor.id, orderId, { 
      email: customerEmail,
      name: customerName,
      phone: customerPhone
    });
  ...
  async findRecommendedVendors(moveDetails, limit = 5) {
    ...
    const availableVendors = allVendors.filter(v => v.specialties.includes(JSON.stringify(['available'])));
    ...
    const scoredVendors = availableVendors.map(vendor => {
      ...
      const reviewCountScore = Math.min(vendor.totalRatings || 0, 30) / 2;
  ...
  async updateVendorPerformanceMetrics(vendorId) {
    ...
    // Save updated vendor
    await vendorStorage.update(updatedVendor.id, updatedVendor);
  ...
  async calculateVendorPrice(vendorId, moveDetails) {
    ...
    try {
      ...
      return {
        ...
        isEstimated: false,
        vendorTier: vendor.tier || 'default'
      };
    }
    
    // Apply vendor tier adjustment
    const tierMultiplier = this.getVendorTierPriceMultiplier(vendor.tier || 'default');
  ...
  async trackCommissionDiscountUsage(vendorId, orderId) {
    try {
      ...
      await vendorStorage.update(vendors[vendorIndex].id, vendors[vendorIndex]);
